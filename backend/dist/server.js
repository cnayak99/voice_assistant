...
async function transcribeAudio(audioFilePath) {
    try {
        ...
        const fileHeader = fs_1.default.readFileSync(audioFilePath, { encoding: null }).slice(0, 100);
...
function postProcessTranscription(text) {
    ...
    const containsTargetFragments = targetPhraseFragments.some(fragment => processedText.toLowerCase().includes(fragment.toLowerCase()));
...
async function processWithGroqContext(messages, abortSignal) {
    try {
...
class AudioStreamProcessor {
    constructor(clientState) {
        ...
        this.maxBufferSize = 30; // Increased from 20 to capture even longer statements
        this.silenceTimeoutMs = 3000; // Increased from 2000 to allow for longer pauses
        this.forceProcessTimeoutMs = 15000; // Increased from 10000 to allow for even longer statements
        this.energyThreshold = 0.005; // Lowered threshold for better voice detection
        this.vadActive = false;
        this.lastVoiceActivityTime = 0;
        this.voiceActivityState = 'SILENCE';
        this.clientState = clientState;
    }
    addChunk(chunk, sequence, timestamp) {
    ...
    detectVoiceActivity(audioData) {
        try {
            ...
            if (this.clientState.ws.readyState === ws_1.WebSocket.OPEN) {
    ...
    async processAudioForTranscription() {
        ...
        try {
            ...
            fs_1.default.writeFileSync(filepath, combinedBuffer);
            ...
            try {
                ...
                if (!transcription || transcription === 'No speech detected' || transcription.trim().length === 0) {
                    ...
                    const response = await processWithGroqContext(this.clientState.callSession.getContextForAI(), this.clientState.abortController?.signal);
        ...
        switch (data.type) {
            ...
            case 'audio_chunk':
                ...
                try {
                    ...
                    clientState.audioProcessor.addChunk(audioBuffer, data.sequenceNumber || 0, data.timestamp || Date.now());
                }
                catch (error) {
...
async function checkAssemblyAIApiKey() {
    try {
